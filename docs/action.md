...
`.github/workflows/release.yml` runs only on a pushed `v*` tag. It drafts the
...
`tests/cli.rs` drives `scripts/action.sh` directly for argument mapping, the
