...
#   ORDNUNG_INSTALL_DIR where to put the binary. Default: ~/.local/bin
#   ORDNUNG_BASE_URL    where to fetch release assets from
...
download_asset() {
    asset_name=$1
    asset_output=$2
    if [ -n "${ORDNUNG_BASE_URL:-}" ]; then
        fetch "${ORDNUNG_BASE_URL}/${asset_name}" >"$asset_output"
...
main() {
  ...
  ORDNUNG_VERSION, which skips the API entirely."
        [ -n "$version" ] || die "no published release found. Set ORDNUNG_VERSION to install a specific tag"
    fi

    # Releases are tagged vX.Y.Z. Accepting X.Y.Z as well means the obvious
    # value for ORDNUNG_VERSION does the obvious thing, rather than asking
    ...
    name="ordnung-${version}-${target}.tar.gz"
    install_dir=${ORDNUNG_INSTALL_DIR:-"${HOME}/.local/bin"}
    ...
    before=$(installed_version "${install_dir}/ordnung")
    if [ -n "$before" ] && [ "v${before}" = "$version" ]; then
        say "already at ${before} in ${install_dir}/ordnung; nothing to do"
        path_note
        return 0
    fi

    work=$(mktemp -d)
    # The half-installed binary goes too. Leaving ordnung.new behind means
    # the next run finds a file it did not write and cannot reason about.
    trap 'rm -rf "$work"; rm -f "${install_dir}/ordnung.new"' EXIT INT TERM
    ...
    [ -f "${work}/ordnung" ] || die "${name} did not contain a ordnung binary"

    mkdir -p "$install_dir"
    # Install by rename so a ordnung that is currently running is replaced
    # rather than written through, which on Linux is a "text file busy" failure.
    mv "${work}/ordnung" "${install_dir}/ordnung.new"
    chmod 755 "${install_dir}/ordnung.new"
    mv "${install_dir}/ordnung.new" "${install_dir}/ordnung"
    ...
    installed=$("${install_dir}/ordnung" --version 2>/dev/null) || die \
"installed ${install_dir}/ordnung, but it would not run.
  The archive was built for ${target}, which is what this machine reports.
  If that is wrong, install with \`cargo install ordnung\` instead."
    ...
    # ORDNUNG_VERSION to walk backwards deserves to see that they did.
    # `replaced` rather than `updated` because this moves either direction,
    # and the two versions say which.
    if [ -n "$before" ]; then
        say "replaced ${before} with $(installed_version "${install_dir}/ordnung") in ${install_dir}/ordnung"
    else
        say "installed ${installed} to ${install_dir}/ordnung"
    fi
    path_note
}
