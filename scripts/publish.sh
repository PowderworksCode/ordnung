#!/usr/bin/env bash
# Publish Ordnung's crates to crates.io, in dependency order.
#
...
# that.
#
#   - Two crates go up, lowest first: ordnung-core, then ordnung-cli, which
#     depends on it. crates.io will not accept a crate whose dependency it does
#     not yet carry, and a version becomes resolvable only once its upload is
#     indexed, so --execute waits for each one to appear before moving on.
#   - The binary is named `ordnung`, but the crate cannot be: that name is taken
#     on crates.io by an unrelated 2020 crate. `cargo install ordnung-cli`
#     installs a binary called `ordnung`.
...
#
# Usage:
#   scripts/publish.sh                    # dry run: package and verify both
...
# That copy publishes one crate named for its repository; this repository is a
# workspace publishing two, neither of which is named `ordnung`.
#
...
CRATES=(ordnung-core ordnung-cli)
MODE=dry-run
REGISTRY=""
...
publish: --execute needs $TOKEN_VAR and it is not set.
  ...
  In CI:   Settings > Secrets and variables > Actions > New repository secret
           ...
           Value: a crates.io API token with publish-new, scoped to
                  ${CRATES[*]}

  Once the crates exist on crates.io, configure Trusted Publishing instead
...
# A just-uploaded crate is not immediately resolvable, and the next crate in the
# order depends on it, so each upload is followed by a wait on the index rather
# than an optimistic push. Two minutes is generous for a registry that is
# normally ready in seconds; failing loudly beats leaving half a release up.
wait_for_index() {
  local crate=$1 waited=0
  while ! already_published "$crate"; do
    if [ "$waited" -ge 120 ]; then
      echo "publish: ${crate} ${VERSION} did not appear on the index within ${waited}s" >&2
      return 1
    fi
    sleep 5
    waited=$((waited + 5))
  done
  echo "publish: ${crate} ${VERSION} is on the index after ${waited}s"
}

for CRATE in "${CRATES[@]}"; do
  # Only --execute is skipped for a version the registry already has: that
  # upload can never succeed and re-running a release for an existing tag
  # should be a no-op rather than a failure.
  if [ "$MODE" = execute ] && already_published "$CRATE"; then
  ...
  # A dry run uploads nothing, so there is nothing to wait for; and the last
  # crate has no dependent behind it.
  if [ "$MODE" = execute ] && [ "$CRATE" != "${CRATES[-1]}" ]; then
